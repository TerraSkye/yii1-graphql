...
class GraphQL {
  ...
  public function schema($schema = NULL) {
    if (is_array($schema)) {
      $schemaQuery = isset($schema['query']) ? $schema['query'] : [];
      $schemaMutation = isset($schema['mutation']) ? $schema['mutation'] : [];
      $schemaTypes = isset($schema['types']) ? $schema['types'] : [];
  ...
  public function buildSchema($schema = NULL) {
    ...
    if ($schema === NULL) {
      list($schemaQuery, $schemaMutation, $schemaTypes) = [
        $this->queries,
        $this->mutations,
        $this->types
      ];
    ...
    $result = new Schema(array_filter([
      'query' => $query,
      'mutation' => $mutation,
      'types' => $types
    ], function ($objectType) {
      ...
      if ($objectType instanceof ObjectType) {
        return !empty($objectType->getFields());
      };
  ...
  protected function buildObjectTypeFromFields($fields, $opts = []) {
    ...
    foreach ($fields as $name => $field) {
      if (is_string($field)) {
        $field = Yii::createComponent($field);
  ...
  public static function type($name) {
    ...
    $module = Yii::app()->controller ? Yii::app()->controller->module : Yii::app()
      ->getModule('graphql');
  ...
  protected function buildType($type) {
    ...
    throw new CException("Type:{$type} is not support translate to Graph Type");
  }
  ...
  protected function getTypeName($class, $name = NULL) {
    ...
    $type = is_object($class) ? $class : Yii::createComponent($class);
    return $type->name;
  }
